...
public class DefaultTaskScheduler extends AbstractTaskScheduler {
  ...
  public class HostVolumeMapping {
    ...
    private HashMap<Integer, LinkedList<QueryUnitAttempt>> unassignedTaskForEachVolume =
        new HashMap<Integer, LinkedList<QueryUnitAttempt>>();
     ...
     *
     * These disk volumes are kept in an order of descending order of the volume id.
     * In other words, the tail volume ids are likely to -1, meaning no given volume id.
     */
    private NavigableMap<Integer, Integer> diskVolumeLoads = new TreeMap<Integer, Integer>().descendingMap();
    ...
    public synchronized void addQueryUnitAttempt(int volumeId, QueryUnitAttempt attemptId){
      LinkedList<QueryUnitAttempt> list = unassignedTaskForEachVolume.get(volumeId);
      if (list == null) {
        list = new LinkedList<QueryUnitAttempt>();
    ...
    private QueryUnitAttemptId getAndRemove(int volumeId){
      QueryUnitAttemptId queryUnitAttemptId = null;
      if(!unassignedTaskForEachVolume.containsKey(volumeId)) return queryUnitAttemptId;

      LinkedList<QueryUnitAttempt> list = unassignedTaskForEachVolume.get(volumeId);
      if(list != null && list.size() > 0){
        QueryUnitAttempt queryUnitAttempt = list.removeFirst();
    ...
    private synchronized void removeQueryUnitAttempt(int volumeId, QueryUnitAttempt queryUnitAttempt){
      ...
      LinkedList<QueryUnitAttempt> tasks  = unassignedTaskForEachVolume.get(volumeId);
