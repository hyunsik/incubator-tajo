...
public class QueryMasterManager extends CompositeService implements EventHandler {
  ...
  private Map<QueryId, QueryMaster> queryMasterTasks = Maps.newConcurrentMap();

  private Map<QueryId, QueryMaster> finishedQueryMasterTasks = Maps.newConcurrentMap();
  ...
  public QueryMasterManager(TajoWorker.WorkerContext workerContext) {
  ...
  public QueryMaster getQueryMasterTask(QueryId queryId) {
    return queryMasterTasks.get(queryId);
  }

  public QueryMaster getQueryMasterTask(QueryId queryId, boolean includeFinished) {
    QueryMaster queryMaster =  queryMasterTasks.get(queryId);
    if(queryMaster != null) {
      return queryMaster;
    } else {
      if(includeFinished) {
  ...
  public Collection<QueryMaster> getQueryMasterTasks() {
    return queryMasterTasks.values();
  }

  public Collection<QueryMaster> getFinishedQueryMasterTasks() {
  ...
  public class QueryMasterContext {
    ...
    public QueryMasterManager getQueryMaster() {
      return QueryMasterManager.this;
    }
    ...
    public void stopQuery(QueryId queryId) {
      ...
      queryMaster = queryMasterTasks.remove(queryId);
      finishedQueryMasterTasks.put(queryId, queryMaster);

      if(queryMaster != null) {
        TajoHeartbeat queryHeartbeat = buildTajoHeartBeat(queryMaster);
  ...
  private TajoHeartbeat buildTajoHeartBeat(QueryMaster queryMaster) {
    TajoHeartbeat queryHeartbeat = TajoHeartbeat.newBuilder()
        .setTajoWorkerHost(workerContext.getQueryMasterManagerService().getBindAddr().getHostName())
        .setTajoQueryMasterPort(workerContext.getQueryMasterManagerService().getBindAddr().getPort())
        .setPeerRpcPort(workerContext.getPeerRpcPort())
        .setTajoWorkerClientPort(workerContext.getTajoWorkerClientService().getBindAddr().getPort())
        .setState(queryMaster.getState())
        .setQueryId(queryMaster.getQueryId().getProto())
        .setQueryProgress(queryMaster.getQuery().getProgress())
        .setQueryFinishTime(queryMaster.getQuery().getFinishTime())
        .build();
  ...
  private class QueryStartEventHandler implements EventHandler<QueryStartEvent> {
    ...
    public void handle(QueryStartEvent event) {
      ...
      QueryMaster queryMaster = new QueryMaster(queryMasterContext,
          event.getQueryId(), event.getQueryContext(), event.getSql(), event.getLogicalPlanJson());

      queryMaster.init(systemConf);
      queryMaster.start();
      synchronized(queryMasterTasks) {
        queryMasterTasks.put(event.getQueryId(), queryMaster);
      }
    }
  }
  ...
  class QueryHeartbeatThread extends Thread {
    ...
    public void run() {
      ...
      while(!queryMasterStop.get()) {
        List<QueryMaster> tempTasks = new ArrayList<QueryMaster>();
        synchronized(queryMasterTasks) {
          tempTasks.addAll(queryMasterTasks.values());
        }
        synchronized(queryMasterTasks) {
          for(QueryMaster eachTask: tempTasks) {
  ...
  class FinishedQueryMasterTaskCleanThread extends Thread {
    ...
    private void cleanExpiredFinishedQueryMasterTask(long expireTime) {
      synchronized(finishedQueryMasterTasks) {
        ...
        for(Map.Entry<QueryId, QueryMaster> entry: finishedQueryMasterTasks.entrySet()) {
