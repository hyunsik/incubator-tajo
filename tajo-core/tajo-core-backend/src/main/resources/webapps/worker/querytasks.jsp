...
<div class='contents'>
  ...
  <hr/>
  <p/>
  <pre><%=subQuery.getBlock().getPlan().toString()%></pre>
  <p/>
  <table border="1" width="100%" class="border_table">
    ...
    <tr><td align='right'>Input Bytes:</td><td><%=nf.format(totalInputBytes)%></td></tr>
    <tr><td align='right'>Read Bytes:</td><td><%=totalReadBytes == 0 ? "-" : nf.format(totalReadBytes)%></td></tr>
    <tr><td align='right'>Input Rows:</td><td><%=nf.format(totalReadRows)%></td></tr>
    <tr><td align='right'>Output Bytes:</td><td><%=nf.format(totalWriteBytes)%></td></tr>
