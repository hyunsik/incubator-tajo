...
public class TajoCli {
        ...
        if (!response.hasTableDesc() && !response.hasResultSet()) {
          ...
          try {
            if (response.hasTableDesc()) {
              resultSet = new TajoResultSet(client, QueryIdFactory.NULL_QUERY_ID, conf,
                  new TableDesc(response.getTableDesc()), response.getMaxRowNum());
              numBytes = 0;
            } else {
              resultSet = TajoClient.createMemoryResultSet(response.getResultSet(), response.getMaxRowNum());
  ...
  private void printResult(ResultSet res, long rowNum, long numBytes) throws IOException, SQLException  {
    try {
      ...
      sout.println("result: " + rowNum + " rows (" + volume + ")");
